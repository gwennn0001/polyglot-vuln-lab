...
def search_users():
    ...
    # INTENTIONAL SQL INJECTION: user input is concatenated directly into SQL.
    sql = f"SELECT id, username, email, role FROM users WHERE username LIKE '%{term}%' OR email LIKE '%{term}%'"
    try:
        with sqlite3.connect(DB_PATH) as connection:
